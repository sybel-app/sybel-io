              ...
              https.get(rssUrl as string, function(res) {
                ...
                res.statusCode < 400
                  ...
                  res.on("end", async function() {
                    ...
                    const background =
`linear-gradient(rgb(${paletteData.DarkMuted?
paletteData.DarkMuted.rgb[0] :
"#fff"}, ${paletteData.DarkMuted?
paletteData.DarkMuted.rgb[1] :
"#fff"}, ${paletteData.DarkMuted?
    paletteData.DarkMuted.rgb[2] :
"#fff"}), rgb(${paletteData.DarkVibrant?
paletteData.DarkVibrant.rgb[0] :
"#fff"},${paletteData.DarkVibrant?
...
"#fff"}))`;
                    const mainColor =
`rgb(${paletteData.Vibrant?
paletteData.Vibrant.rgb[0]:
"#fff"},${paletteData.Vibrant?
paletteData.Vibrant.rgb[1]:
"#fff"},${paletteData.Vibrant?
paletteData.Vibrant.rgb[2]:
"#fff"})`;
                    response
                        .status(200)
                        .send({title,
